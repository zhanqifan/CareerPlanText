...
Name"
      ...
      <el-form-item label="目标状态(0:已发布，1:已废止，2:草稿)" prop="state">
        <el-input
          ...
          placeholder="请输入目标状态(0:已发布，1:已废止，2:草稿)"
          clearable
          @keyup.enter.native="handleQuery"
        />
      </el-form-item>
      ...
      <el-form-item label="修改次数" prop="modificationsNumber">
        <el-input
          v-model="queryParams.modificationsNumber"
          placeholder="请输入修改次数"
          clearable
          @keyup.enter.native="handleQuery"
        />
      ...
      <el-form-item label="批阅次数" prop="reviewsNumber">
        <el-input
          v-model="queryParams.reviewsNumber"
          placeholder="请输入批阅次数"
          clearable
          @keyup.enter.native="handleQuery"
        />
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['student:position:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['student:position:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['student:position:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['student:position:export']"
        >导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="positionList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="岗位id" align="center" prop="position
Id" />
...
Name: null,
    ...
    handleSelectionChange(selection) {
      this.ids = selection.map(item => item.position
Id)
...
Ids = row.position
Id || this.ids;
      this.$modal.confirm('是否确认删除岗位管理编号为"' + position
Ids + '"的数据项？').then(function() {
        return delPosition(position
Ids);
      }).then(() => {
        this.getList();
